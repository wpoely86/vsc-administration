...
from vvsc.utils.generaloption import simple_option
...
def getAllDBusers(debug=None):
    c = cCol()
    c.debug = debug
    users = []
    currentUsers = c.getlist("member", "uid, inst, active")
    for curr_user in currentUsers:
        if curr_user[1] == 'gent':
            users.append(user(vscid=curr_user[0], active=curr_user[2]))
...
if __name__ == '__main__':
    ...
    users = getAllDBusers()
    users = addAllUGID(users)
    users = addEmplStudent(users)
